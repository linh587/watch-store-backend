...
export async function getCart(userAccountId: string, limit?: LimitOptions) {
  ...
  const [cartDetailRowDatas] = (await pool.query(getCartQuery, [
    userAccountId,
  ])) as RowDataPacket[][];
  return cartDetailRowDatas.map(convertUnderscorePropertiesToCamelCase) as Cart;
}

export async function getExtraCartDetail(
  ...
  const [cartDetailItemRowDatas] = (await pool.query(getCartDetailItemQuery, [
    userAccountId,
    productPriceId,
  ])) as RowDataPacket[][];
  return convertUnderscorePropertiesToCamelCase(
    cartDetailItemRowDatas[0] || null
  ) as ExtraCartDetail | null;
}

export async function updateCartDetail(
  userAccountId: string,
  information: InformationToUpdateCartDetail
) {
  ...
  const [result] = (await pool.query(updateCartDetailItemQuery, [
    quality,
    userAccountId,
    productPriceId,
    productPriceId,
...
export async function addToCart(
  userAccountId: string,
  information: InformationToCreateCartDetail
) {
  try {
    const { productPriceId, quality } = information;
    const addToCartQuery =
      "insert into cart_detail(`user_account_id`, `product_price_id`, `quality`) values (?) on duplicate key update quality=quality + ?";
    const [result] = (await pool.query(addToCartQuery, [
      [userAccountId, productPriceId, quality],
      quality,
    ])) as OkPacket[];
    return result.affectedRows > 0;
  } catch (error) {
...
export async function deleteCartDetail(
  userAccountId: string,
  productPriceId: string
) {
  const deleteCartDetailItemQuery =
    "delete from cart_detail where user_account_id=? and product_price_id=?";
  const [result] = (await pool.query(deleteCartDetailItemQuery, [
    userAccountId,
    productPriceId,
  ])) as OkPacket[];
  return result.affectedRows > 0;
