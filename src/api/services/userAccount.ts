...
export async function signIn(email: string, password: string) {
  const findUserAccountQuery =
    "select id, verified from user_account where email=? and password=? and deleted_at is null and verified=true";
...
export async function getUserAccounts(limit?: LimitOptions) {
  ...
  return userAccountRowDatas.map(
    convertUnderscorePropertiesToCamelCase
  ) as ShortUserAccount[];
}
...
export async function getInformation(userAccountId: string) {
  ...
  if (
    Array.isArray(userInformationRowDatas) &&
    userInformationRowDatas.length > 0
  ) {
    // remap data
    userInformationRowDatas[0].gender = decodeGender(
      userInformationRowDatas[0].gender
    );
    return convertUnderscorePropertiesToCamelCase(
      userInformationRowDatas[0] || null
    ) as UserAccount | null;
  }
...
export async function updateInformation(
  ...
  information: InformationToUpdateUserAccount
) {
  const {
    email,
    name,
...
export async function deleteAccount(userAccountId: string) {
  ...
  try {
    ...
    const deleteAccountQuery =
      "update user_account set deleted_at=? where id=? and deleted_at is null";
...
export async function forceUpdatePassword(
  userAccountId: string,
  password: string
) {
  const updatePasswordQuery =
    "update user_account set password=? where id=? and deleted_at is null";
  const hashedPassword = hashText(password);
  try {
    const [result] = (await pool.query(updatePasswordQuery, [
      hashedPassword,
...
export async function getId(email: string) {
  const getIdQuery =
    "select id from user_account where email=? and deleted_at is null";
  const [result] = (await pool.query(getIdQuery, [email])) as RowDataPacket[];
  return result.length > 0 ? result[0].id : "";
}

export async function createAccount(
  information: InformationToCreateUserAccount
) {
...
export async function verifyEmail(id: string) {
  const verifyEmailQuery =
    "update user_account set verified=true where id=? and deleted_at is null";
...
export async function lockAccount(userAccountId: string) {
  const lockAccountQuery =
    "update user_account set locked=true where id=? and locked=false and deleted_at is null";
...
export async function unlockAccount(userAccountId: string) {
  const lockAccountQuery =
    "update user_account set locked=false where id=? and locked=true and deleted_at is null";
  const [result] = (await pool.query(lockAccountQuery, [
    userAccountId,
  ])) as OkPacket[];
  return result.affectedRows > 0;
...
export async function checkLock(userAccountId: string) {
  const checkLockQuery =
    "select count(id) as locked_count from user_account where id=? and locked=true";
