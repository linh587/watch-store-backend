...
export async function getCoupons(limit?: LimitOptions) {
  let getCouponsQuery =
    "select coupon_code, type, begin_at, finish_at, decrease, unit, applied_scopes from coupon";

  if (limit) {
    getCouponsQuery += " " + createLimitSql(limit);
  }

  const [couponRowDatas] = (await pool.query(
    getCouponsQuery
  )) as RowDataPacket[][];
...
export async function search(text: string) {
  const searchCouponQuery =
    "select coupon_code, type, begin_at, finish_at, decrease, unit, applied_scopes from coupon where coupon_code like ?";
...
export async function getCoupon(couponCode: string) {
  const getCouponQuery =
    "select coupon_code, type, begin_at, finish_at, decrease, unit, applied_scopes from coupon where coupon_code=?";
  const [couponRowDatas] = (await pool.query(getCouponQuery, [
    couponCode,
  ])) as RowDataPacket[][];
  const coupon = convertUnderscorePropertiesToCamelCase(
    couponRowDatas[0] || null
  ) as Coupon | null;
  if (coupon) {
    const appliedScopes = String(
      couponRowDatas[0]?.["applied_scopes"] || ""
    ).split(".");
    let couponScopes = {};

    if (appliedScopes.includes("order")) {
      const orderCondition = await CouponOnOrderService.getOrderCondition(
        couponCode
      );
    ...
    if (appliedScopes.includes("product")) {
      const productPriceIds = await CouponOnProductService.getProductPriceIds(
        couponCode
      );
...
export async function addCoupon(information: InformationToCreateCoupon) {
  const couponCode = String(
    information?.["couponCode"] ||
      createUid(LENGTH_OF_COUPON_CODE).toUpperCase()
  );
  const addCouponQuery =
    "insert into coupon(`coupon_code`, `type`, `begin_at`, `finish_at`, `decrease`, `unit`, `applied_scopes`) values (?)";
  ...
  if (
    appliedScopes.some(
      (scope) =>
        !APPLIED_SCOPES.includes(scope as (typeof APPLIED_SCOPES)[number])
    )
  ) {
    return false;
  }
  ...
  try {
    ...
    await poolConnection.query(addCouponQuery, [
      [
        couponCode,
        type,
        new Date(beginAt as string),
        new Date(finishAt as string),
        decrease,
        unit,
        appliedScopes.join("."),
      ],
    ...
    if (appliedScopes.includes("product")) {
      ...
      await CouponOnProductService.addProductPrices(
        couponCode,
        productPriceIds,
        poolConnection
      );
...
export async function updateCoupon(
  couponCode: string,
  information: InformationToUpdateCoupon
) {
  ...
  const updateCouponQuery =
    "update coupon set begin_at=?, finish_at=?, decrease=?, unit=?, type=?, applied_scopes=? where coupon_code=? and begin_at>?";
  ...
  try {
    ...
    if (appliedScopes.includes("order")) {
      ...
      await CouponOnOrderService.deleteOrderCondition(
        couponCode,
        poolConnection
      );
    }
    ...
    if (appliedScopes.includes("product")) {
      ...
      await CouponOnProductService.updateProductPrices(
        couponCode,
        productPriceIds,
        poolConnection
      );
    } else {
      await CouponOnProductService.deleteProductPrices(
        couponCode,
        poolConnection
      );
    }
...
export async function deleteCoupon(couponCode: string) {
  const deleteCouponQuery =
    "delete from coupon where coupon_code=? and begin_at>?";
...
export function calculateDecreaseMoneyForOrder(
  coupon: Coupon,
  order: OrderService.TemporaryOrder
) {
  ...
  if (coupon.type === "order") {
    ...
    return calculateDecreaseMoneyByUnit(
      totalPrice,
      coupon.decrease,
      coupon.unit
    );
  ...
  if (coupon.type === "product") {
    const productPriceIdsMatchedCondition = coupon.appliedScopes.includes(
      "product"
    )
      ? CouponOnProductService.getMatchedProductPriceIds(
          coupon as CouponOnProductService.CouponOnProduct,
          order
        )
    ...
    const totalDecreasePrice = order.details
      .filter(({ productPriceId }) =>
        productPriceIdsMatchedCondition.includes(productPriceId)
      )
      .map(
        ({ price, quality }) =>
          calculateDecreaseMoneyByUnit(price, coupon.decrease, coupon.unit) *
          quality
      )
...
function matchCondition(coupon: Coupon, order: OrderService.TemporaryOrder) {
  ...
  if (appliedScopes.includes("order")) {
    if (
      !CouponOnOrderService.matchCondition(
        coupon as CouponOnOrderService.CouponOnOrder,
        order
      )
    ) {
      return false;
  ...
  if (appliedScopes.includes("product")) {
    if (
      !CouponOnProductService.matchCondition(
        coupon as CouponOnProductService.CouponOnProduct,
        order
      )
    ) {
...
function calculateDecreaseMoneyByUnit(
  price: number,
  decrease: number,
  unit: CouponUnit
) {
...
export async function getRelationCouponCodes(
  order: OrderService.TemporaryOrder
) {
  const totalPriceOfOrder = OrderService.calculateTemporaryTotalPrice(
    order.details
  );
  const productPriceIdsOfOrder = order.details.map(
    ({ productPriceId }) => productPriceId
  );
  const relationCouponCodesOnOrder =
...
export async function findRelationCoupons(order: OrderService.TemporaryOrder) {
  ...
  return relationCoupons
    ...
    .filter(
      (coupon) => validateCoupon(coupon) && matchCondition(coupon, order)
    );
}
