...
import {
  ...
  EXPIRE_TIME_OF_RESET_PASSWORD_TOKEN,
} from "../config.js";
...
export async function signInAdmin(req: Request, res: Response) {
  ...
  const adminSignInResult = await AdminAccountService.signIn(
    username,
    password
  );
...
export async function forgotPassword(req: Request, res: Response) {
  ...
  const resetPasswordToken = jwt.sign(
    ...
    { expiresIn: EXPIRE_TIME_OF_RESET_PASSWORD_TOKEN }
  );
  const mailContent =
...
export async function resetPassword(req: Request, res: Response) {
  ...
  try {
    ...
    const success = await UserAccountService.forceUpdatePassword(
      userId,
      newPassword
    );
