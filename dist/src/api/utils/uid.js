export function createUid(size = 20) {
    ...
    const headToken = getLastSubString(Date.now().toString(36), headTokenLength).padStart(headTokenLength, '0');
    const tailToken = getLastSubString(Math.random().toString(36).replace(/\./g, ''), tailTokenLength).padEnd(tailTokenLength, '0');
    return headToken + tailToken;
}
function getLastSubString(s, size = 10) {
